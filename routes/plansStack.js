            ...
            onPress={async () => {
              if (Constants.platform.ios) {
                ...
                if (status !== "granted" /*&& status2 !== 'granted'*/) {
                  ...
                  plans.getAddress(plan.ids[0].latitude, plan.ids[0].longitude, data => {
                    const eventConfig = {
                      title: plan.title,
                      startDate: new Date(plan.startUNIX * 1000).toISOString(),
                      endDate: new Date((plan.startUNIX + 3600*plan.ids.length) * 1000).toISOString(),
                      location: data.results[0].formatted_address,
                      url: 'https://app.exire.ai/plan/' + plan.planID,
                      timezone: new Date().getTimezoneOffset(),
                      notes: plan.description
                    };
                    Calendar.getDefaultCalendarAsync().then(cal => {
                      // Calendar.getCalendarsAsync().then(cal => {
                      Calendar.createEventAsync(cal.id, eventConfig)
                      .then(eventInfo => {
                        Alert.alert("Event Created", "Added to your " + cal.source.name + " calendar!")
                      })
                      .catch((error) => {
                        console.warn(error);
                      })
